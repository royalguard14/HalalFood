...
class _DriverDashboardScreenState extends State<DriverDashboardScreen> {
  ...
  Widget _availableDeliveriesCard() {
    if (_loadingDeliveries) return const Center(child: CircularProgressIndicator());
      ...
      return Card(
        ...
        child: Padding(
          ...
          child: Column(
            ...
            children: [
              Icon(offline ? Icons.wifi_off_rounded : Icons.local_shipping_outlined,
                  size: 46, color: Colors.grey.shade400),
              const SizedBox(height: 10),
              Text(
                offline ? 'Go Online to See Deliveries' : 'No available delivery',
                style: const TextStyle(fontWeight: FontWeight.w800),
              ),
              const SizedBox(height: 5),
              Text(
    ...
    return ListView.separated(
      ...
      itemBuilder: (context, index) => _availableDeliveryCard(_availableDeliveries[index]),
    );
  }

  Widget _availableDeliveryCard(Map<String, dynamic> delivery) {
    final orderId = delivery['order_id']?.toString() ?? '';
    final shortOrderId = orderId.length > 8 ? orderId.substring(0, 8).toUpperCase() : orderId.toUpperCase();
    final restaurant = Map<String, dynamic>.from(delivery['restaurant'] ?? const {});
    final advance = _toAmount(delivery['restaurant_food_advance']);
    final collection = _toAmount(delivery['customer_collection_amount']);
          ...
          children: [
            Row(
              children: [
                ...
                Text('#$shortOrderId', style: const TextStyle(fontSize: 12, fontWeight: FontWeight.w700)),
              ],
            ),
            const SizedBox(height: 12),
            if (advance != null) _deliveryAmountRow('Restaurant advance', '₱${advance.toStringAsFixed(2)}'),
            if (collection != null) _deliveryAmountRow('Customer collection', '₱${collection.toStringAsFixed(2)}'),
            _deliveryAmountRow(
              'Rider → Restaurant',
              delivery['rider_to_restaurant_distance']?.toString() ?? 'Location unavailable',
            ),
            _deliveryAmountRow(
              'Restaurant → Customer',
              delivery['restaurant_to_customer_distance']?.toString() ?? 'Location unavailable',
            ),
            const SizedBox(height: 12),
            SizedBox(
              width: double.infinity,
        ...
        child: Padding(
          ...
          child: Column(
            children: [
              ...
              const SizedBox(height: 10),
              const Text('No active deliveries', style: TextStyle(fontWeight: FontWeight.w800)),
              const SizedBox(height: 5),
              const Text(
                'Deliveries you accept will stay here until completed.',
                textAlign: TextAlign.center,
          ...
          child: Padding(
            ...
            child: Column(
              ...
              children: [
                ...
                const SizedBox(height: 12),
                if (restaurant['name'] != null) _deliveryAmountRow('Restaurant', restaurant['name'].toString()),
                _deliveryAmountRow('Status', _riderStatusLabel(delivery['status']?.toString())),
                if (advance != null) _deliveryAmountRow('Restaurant advance', '₱${advance.toStringAsFixed(2)}'),
                if (collection != null) _deliveryAmountRow('Customer collection', '₱${collection.toStringAsFixed(2)}'),
              ],
            ),
  ...
  String _riderStatusLabel(String? status) {
    switch (status) {
      ...
      case 'rider_going_to_restaurant': return 'Going to Restaurant';
      case 'rider_at_restaurant': return 'At Restaurant';
      case 'picked_up': return 'Picked Up';
      case 'out_for_delivery': return 'Out for Delivery';
      case 'delivered_cash_collected': return 'Delivered / Cash Collected';
