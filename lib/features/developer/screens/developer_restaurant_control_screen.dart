...
class _DeveloperRestaurantControlScreenState extends State<DeveloperRestaurantControlScreen> {
  ...
  Future<void> _load() async {
    ...
    try {
      ...
      final ids=(orders as List).map((r)=>r['id']?.toString()).whereType<String>().toList();
      ...
      final out=(cashouts as List).fold<double>(0,(s,r)=>s+((r['amount'] as num?)?.toDouble()??0));
      double ca=0,ga=0;
      ...
      if(!mounted)return;
      setState((){_cashReceived=cash;_gcashReceived=gcash;_gcashCashouts=out;_cashAdjustments=ca;_gcashAdjustments=ga;_adjustments=(adjustments as List).map((r)=>Map<String,dynamic>.from(r)).toList();_loading=false;});
