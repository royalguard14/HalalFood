...
class _AdminDashboardScreenState extends State<AdminDashboardScreen> {
  ...
  Widget _buildBody() {
    ...
    if (_error != null) return ListView(physics: const AlwaysScrollableScrollPhysics(), padding: const EdgeInsets.all(24), children: [const SizedBox(height: 120), const Icon(Icons.cloud_off_rounded, size: 56, color: Colors.redAccent), const SizedBox(height: 16), const Text('Unable to load dashboard', textAlign: TextAlign.center, style: TextStyle(fontSize: 20, fontWeight: FontWeight.w800)), const SizedBox(height: 8), Text(_error!, textAlign: TextAlign.center), const SizedBox(height: 16), ElevatedButton(onPressed: _loadDashboard, child: const Text('Try Again'))]);
    return LayoutBuilder(builder: (context, constraints) {
      final wide = constraints.maxWidth >= 900;
      return ListView(physics: const AlwaysScrollableScrollPhysics(), padding: EdgeInsets.fromLTRB(wide ? 32 : 18, 22, wide ? 32 : 18, 40), children: [_headerCard(wide), const SizedBox(height: 24), _sectionTitle('Platform Overview', 'Live summary of your HALAL Food platform'), const SizedBox(height: 12), _stats(wide), const SizedBox(height: 24), if (_pendingVerificationCount > 0) ...[_verificationAlert(), const SizedBox(height: 24)], _sectionTitle('Admin Tools', 'Quick access to the areas you manage most'), const SizedBox(height: 12), _tools(wide)]);
    });
  }
  ...
  Widget _stats(bool wide) {
    ...
    if (wide) return Wrap(spacing: 12, runSpacing: 12, children: cards.map((c) => SizedBox(width: 220, child: c)).toList());
