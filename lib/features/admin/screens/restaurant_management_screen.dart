...
class _RestaurantManagementScreenState extends State<RestaurantManagementScreen> {
  ...
  List<Map<String, dynamic>> get _filtered => _restaurants.where((r) {
    ...
    final matchesSearch = q.isEmpty || name.contains(q) || city.contains(q) || owner.contains(q);
  ...
  Future<void> _addRestaurant() async {
    final saved = await showModalBottomSheet<bool>(
      ...
      showDragHandle: true,
      builder: (_) => _RestaurantForm(owners: _owners, repository: _restaurantRepository),
    );
    if (saved == true) {
      await _load();
  ...
  Future<void> _editRestaurant(Map<String, dynamic> restaurant) async {
    final saved = await showModalBottomSheet<bool>(
      ...
      isScrollControlled: true,
      showDragHandle: true,
      builder: (_) => _RestaurantForm(owners: _owners, repository: _restaurantRepository, restaurant: restaurant),
    );
    if (saved == true) {
      await _load();
  ...
  Future<void> _assignOwner(Map<String, dynamic> restaurant) async {
    ...
    final result = await showDialog<_OwnerSelectionResult>(
      context: context,
      builder: (_) => _OwnerSelectionDialog(owners: _owners, initialOwnerId: selected),
    );
    if (result == null || result.ownerId == selected) return;
    try {
      await _restaurantRepository.assignOwner(restaurantId: restaurant['id'].toString(), ownerId: result.ownerId);
      await _load();
      if (mounted) _message('Restaurant owner updated.');
    } catch (e) { if (mounted) _message('Unable to assign owner: $e'); }
  }

  Future<void> _toggleActive(Map<String, dynamic> restaurant) async {
    final next = restaurant['is_active'] != true;
    try {
      await _repository.setRestaurantActive(restaurantId: restaurant['id'].toString(), isActive: next);
      await _load();
      if (mounted) _message(next ? 'Restaurant activated.' : 'Restaurant deactivated.');
    } catch (e) { if (mounted) _message('Unable to update status: $e'); }
  }

  Future<void> _toggleFeatured(Map<String, dynamic> restaurant) async {
    final next = restaurant['is_featured'] != true;
    try {
      await _repository.setRestaurantFeatured(restaurantId: restaurant['id'].toString(), isFeatured: next);
      await _load();
      if (mounted) _message(next ? 'Restaurant featured.' : 'Restaurant removed from featured.');
    } catch (e) { if (mounted) _message('Unable to update featured status: $e'); }
  }
      ...
      builder: (sheetContext) => SafeArea(
        child: Padding(
          ...
          child: Column(
            ...
            children: [
              Text(restaurant['name']?.toString() ?? 'Restaurant', style: const TextStyle(fontSize: 24, fontWeight: FontWeight.w800)),
              ...
              Row(children: [
                Expanded(child: OutlinedButton.icon(onPressed: () { Navigator.pop(sheetContext); _assignOwner(restaurant); }, icon: const Icon(Icons.person_add_alt_1_rounded), label: const Text('Owner'))),
                const SizedBox(width: 8),
                Expanded(child: ElevatedButton.icon(onPressed: () { Navigator.pop(sheetContext); _editRestaurant(restaurant); }, icon: const Icon(Icons.edit_rounded), label: const Text('Edit'))),
              ]),
            ],
          ),
        ),
                  ...
                  child: ListView(
                    ...
                    children: [
                      ...
                      SizedBox(height: 42, child: ListView(scrollDirection: Axis.horizontal, children: [
                        for (final item in const [('All','all'),('Active','active'),('Inactive','inactive'),('Unassigned','unassigned'),('Unverified','unverified')])
                      ...
                      else ...restaurants.map(_restaurantCard),
                    ],
                  ),
                ),
    );
      ...
      child: InkWell(
        ...
        child: Padding(
          ...
          child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            Row(children: [
              Expanded(child: Text(r['name']?.toString() ?? 'Unnamed Restaurant', maxLines: 2, overflow: TextOverflow.ellipsis, style: const TextStyle(fontSize: 17, fontWeight: FontWeight.w800))),
              if (featured) const Icon(Icons.star_rounded, color: Colors.amber),
            ]),
            if ((r['city']?.toString() ?? '').isNotEmpty) Padding(padding: const EdgeInsets.only(top: 3), child: Text(r['city'].toString(), style: const TextStyle(fontSize: 12, color: HalalFoodTheme.textSecondary))),
            ...
            const SizedBox(height: 12),
            Row(children: [
              Expanded(child: _RestaurantToggleButton(
              ...
              Expanded(child: _RestaurantToggleButton(
                ...
                icon: featured ? Icons.star_rounded : Icons.star_border_rounded,
...
class _RestaurantToggleButton extends StatelessWidget {
  ...
  const _RestaurantToggleButton({required this.label, required this.icon, required this.color, required this.onPressed});

  @override
  Widget build(BuildContext context) => OutlinedButton.icon(
    onPressed: onPressed,
    icon: Icon(icon, size: 19, color: color),
    label: Text(label, style: TextStyle(fontWeight: FontWeight.w700, color: color)),
...
class _RestaurantForm extends StatefulWidget {
  ...
  @override State<_RestaurantForm> createState() => _RestaurantFormState();
}
...
class _RestaurantFormState extends State<_RestaurantForm> {
  ...
  @override
  void dispose() {
    for (final c in [_name,_description,_phone,_email,_website,_address,_city,_province,_latitude,_longitude,_logo,_cover]) { c.dispose(); }
    super.dispose();
  }
  ...
  Future<void> _save() async {
    ...
    try {
      final values = {
        'name': _name.text, 'ownerId': _ownerId, 'description': _description.text,
        'phone': _phone.text, 'email': _email.text, 'website': _website.text,
        'address': _address.text, 'city': _city.text, 'province': _province.text,
        'latitude': double.tryParse(_latitude.text.trim()), 'longitude': double.tryParse(_longitude.text.trim()),
        'logoUrl': _logo.text, 'coverImageUrl': _cover.text,
      };
      if (_editing) {
        await widget.repository.updateRestaurant(
          restaurantId: widget.restaurant!['id'].toString(), name: values['name'] as String,
          ownerId: values['ownerId'] as String?, description: values['description'] as String,
          phone: values['phone'] as String, email: values['email'] as String, website: values['website'] as String,
          address: values['address'] as String, city: values['city'] as String, province: values['province'] as String,
          latitude: values['latitude'] as double?, longitude: values['longitude'] as double?,
          logoUrl: values['logoUrl'] as String, coverImageUrl: values['coverImageUrl'] as String,
        );
      } else {
        await widget.repository.createRestaurant(
          name: values['name'] as String, ownerId: values['ownerId'] as String?, description: values['description'] as String,
          phone: values['phone'] as String, email: values['email'] as String, website: values['website'] as String,
          address: values['address'] as String, city: values['city'] as String, province: values['province'] as String,
          latitude: values['latitude'] as double?, longitude: values['longitude'] as double?,
          logoUrl: values['logoUrl'] as String, coverImageUrl: values['coverImageUrl'] as String,
  ...
  Widget _field(TextEditingController c, String label, IconData icon, {bool required = false, int maxLines = 1, TextInputType? type}) => Padding(
    ...
    child: TextFormField(
      controller: c, maxLines: maxLines, keyboardType: type, enabled: !_saving,
  ...
  Widget build(BuildContext context) {
    ...
    return Padding(
      ...
      child: SafeArea(child: Form(key: _formKey, child: SingleChildScrollView(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Text(_editing ? 'Edit Restaurant' : 'Add New Restaurant', style: const TextStyle(fontSize: 24, fontWeight: FontWeight.w800)),
        const SizedBox(height: 6),
        Text(_editing ? 'Update restaurant information.' : 'New restaurants are created inactive. You can activate them from the restaurant list.', style: const TextStyle(color: HalalFoodTheme.textSecondary)),
        ...
        DropdownButtonFormField<String?>(
          value: _ownerId, isExpanded: true,
          decoration: _decoration('Owner Account', Icons.person_outline_rounded), hint: const Text('Unassigned'),
          items: [const DropdownMenuItem<String?>(value: null, child: Text('Unassigned')), ...widget.owners.map((owner) => DropdownMenuItem<String?>(value: owner['id']?.toString(), child: Text(owner['full_name']?.toString().trim().isNotEmpty == true ? owner['full_name'].toString() : 'Restaurant Owner')))],
...
class _OwnerSelectionResult { final String? ownerId; const _OwnerSelectionResult(this.ownerId); }

class _OwnerSelectionDialog extends StatefulWidget {
  final List<Map<String, dynamic>> owners; final String? initialOwnerId;
  const _OwnerSelectionDialog({required this.owners, required this.initialOwnerId});
  @override State<_OwnerSelectionDialog> createState() => _OwnerSelectionDialogState();
}

class _OwnerSelectionDialogState extends State<_OwnerSelectionDialog> {
  late String? _selected;
  @override void initState() { super.initState(); _selected = widget.initialOwnerId; }
  @override Widget build(BuildContext context) => AlertDialog(
    title: const Text('Assign Restaurant Owner', style: TextStyle(fontWeight: FontWeight.w800)),
    content: DropdownButtonFormField<String?>(
      value: _selected, isExpanded: true, decoration: const InputDecoration(labelText: 'Owner account'),
      items: [const DropdownMenuItem<String?>(value: null, child: Text('Unassigned')), ...widget.owners.map((owner) => DropdownMenuItem<String?>(value: owner['id']?.toString(), child: Text(owner['full_name']?.toString().trim().isNotEmpty == true ? owner['full_name'].toString() : 'Restaurant Owner')))],
      onChanged: (value) => setState(() => _selected = value),
    ),
    actions: [TextButton(onPressed: () => Navigator.pop(context), child: const Text('Cancel')), ElevatedButton(onPressed: () => Navigator.pop(context, _OwnerSelectionResult(_selected)), child: const Text('Save'))],
  );
}
...
class _DetailRow extends StatelessWidget {
  ...
  @override Widget build(BuildContext context) => Padding(padding: const EdgeInsets.symmetric(vertical: 5), child: Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
