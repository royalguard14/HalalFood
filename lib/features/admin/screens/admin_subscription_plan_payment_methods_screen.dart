...
class AdminSubscriptionPlanPaymentMethodsScreen extends StatefulWidget {
  ...
  @override
  State<AdminSubscriptionPlanPaymentMethodsScreen> createState() =>
      _AdminSubscriptionPlanPaymentMethodsScreenState();
}

class _AdminSubscriptionPlanPaymentMethodsScreenState
    extends State<AdminSubscriptionPlanPaymentMethodsScreen> {
  ...
  Future<void> _load() async {
    ...
    try {
      ...
      selected.clear();
      final planIds = loadedPlans.map((p) => p['id'].toString()).toList();
      ...
      if (planIds.isNotEmpty) {
        ...
        for (final row in List<Map<String, dynamic>>.from(links as List)) {
          ...
          selected.putIfAbsent(planId, () => <String>{}).add(methodId);
        }
      }
      ...
      _message('Unable to load plan payment settings: $e');
    }
  }
  ...
  Future<void> _save(String planId) async {
    try {
      ...
      if (ids.isNotEmpty) {
        await db.from('subscription_plan_payment_methods').insert(
              ids
                  .map(
                    (id) => {
                      'plan_id': planId,
      ...
      if (mounted) _message('Unable to save plan payment methods: $e');
    }
  }
      ...
      body: loading
          ...
          : RefreshIndicator(
              ...
              child: ListView(
                ...
                children: [
                  const Text(
                    'Assign Payment Methods to Each SaaS Plan',
                    style: TextStyle(fontSize: 20, fontWeight: FontWeight.w800),
                  ),
                  const SizedBox(height: 6),
                  const Text(
                    'When an owner selects a plan, only the payment methods enabled here will be offered.',
                  ),
                  ...
                  if (methods.isEmpty)
                    const Card(
                      child: Padding(
                        ...
                        child: Text('Add payment methods first.'),
                      ),
                    )
                    ...
                    const Card(
                      child: Padding(
                        padding: EdgeInsets.all(16),
                        child: Text('No active SaaS plans found.'),
                      ),
                    )
                  else
                    ...plans.map(_planCard),
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            Text(
              '₱${plan['monthly_price'] ?? 0} / month • ₱${plan['annual_price'] ?? 0} / year',
            ),
            const SizedBox(height: 10),
            ...methods.map(
              (method) => CheckboxListTile(
                contentPadding: EdgeInsets.zero,
                value: set.contains(method['id'].toString()),
                onChanged: method['is_active'] == true
                    ? (value) {
                        setState(() {
