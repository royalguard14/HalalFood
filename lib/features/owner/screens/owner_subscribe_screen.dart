...
class _OwnerSubscribeScreenState extends State<OwnerSubscribeScreen> {
  ...
  Future<void> _loadPlans() async {
    ...
    try {
      final rows = await _supabase.from('subscription_plans').select('id,name,description,monthly_price,annual_price,features').eq('is_active', true).order('sort_order');
      if (!mounted) return;
  ...
  Future<void> _subscribe(Map<String, dynamic> plan, bool annual) async {
    ...
    try {
      ...
      final end = annual ? DateTime.utc(now.year + 1, now.month, now.day) : DateTime.utc(now.year, now.month + 1, now.day);
  ...
  Widget _buildBody() {
    ...
    if (_error != null) return ListView(physics: const AlwaysScrollableScrollPhysics(), padding: const EdgeInsets.all(24), children: [const SizedBox(height: 100), Text('Unable to load subscription plans.', textAlign: TextAlign.center), const SizedBox(height: 8), Text(_error!, textAlign: TextAlign.center), const SizedBox(height: 16), ElevatedButton(onPressed: _loadPlans, child: const Text('Try Again'))]);
  ...
  Widget _planCard(Map<String, dynamic> plan) {
    ...
    return Card(margin: const EdgeInsets.only(bottom: 14), child: Padding(padding: const EdgeInsets.all(18), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      ...
      const SizedBox(height: 14), Row(children: [Expanded(child: OutlinedButton(onPressed: _subscribing ? null : () => _subscribe(plan, false), child: const Text('Subscribe Monthly'))), const SizedBox(width: 10), Expanded(child: ElevatedButton(onPressed: _subscribing || annual <= 0 ? null : () => _subscribe(plan, true), child: const Text('Subscribe Annual')))]),
    ])));
  }
}
