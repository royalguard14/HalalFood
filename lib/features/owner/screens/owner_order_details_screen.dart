...
class _OwnerOrderDetailsScreenState extends State<OwnerOrderDetailsScreen> {
  ...
  Widget _buildActionButtons() {
    ...
    switch (_status.toLowerCase()) {
      case 'pending': return SizedBox(width: double.infinity, height: 54, child: ElevatedButton.icon(onPressed: _isUpdating ? null : () => _updateStatus('preparing'), icon: _isUpdating ? const SizedBox(width: 18, height: 18, child: CircularProgressIndicator(strokeWidth: 2)) : const Icon(Icons.restaurant_rounded), label: const Text('Accept & Start Preparing', style: TextStyle(fontWeight: FontWeight.w800))));
