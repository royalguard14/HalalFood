...
class _OwnerDashboardScreenState extends State<OwnerDashboardScreen> {
  ...
  Widget _buildDashboard() {
    return ListView(
      ...
      children: [
        ...
        Row(
          children: [
            Expanded(child: _statCard(Icons.restaurant_menu_rounded, 'Menu Items', '—')),
            const SizedBox(width: 12),
            Expanded(child: _statCard(Icons.receipt_long_rounded, 'Open Orders', '${_newOrders + _preparingOrders + _readyOrders}')),
          ],
        ),
        const SizedBox(height: 16),
        SizedBox(
        ...
        Row(
          children: [
            Expanded(child: _statCard(Icons.payments_outlined, 'Sales This Month', '₱${_monthlySales.toStringAsFixed(0)}')),
            const SizedBox(width: 10),
            Expanded(child: _statCard(Icons.receipt_long_outlined, 'Orders This Month', '$_monthlyOrders')),
          ],
        ),
        const SizedBox(height: 24),
  ...
  Widget _buildSubscriptionCard() {
    ...
    final status = _subscriptionStatus == null ? 'NO ACTIVE PLAN' : _subscriptionStatus!.toUpperCase().replaceAll('_', ' ');
              ...
              Expanded(
                child: Column(
                  ...
                  children: [
                    ...
                    Text(status, style: TextStyle(fontWeight: FontWeight.w800, color: color)),
                    if (_subscriptionPlan != null) Text(_subscriptionPlan!),
                    if (_subscriptionExpiry != null) Text('Expires: ${_formatDate(_subscriptionExpiry)}'),
                    if (!active) const Text('Tap to choose a plan', style: TextStyle(fontWeight: FontWeight.w600)),
                  ],
                ),
              ),
        ...
        child: Row(
          children: [
            Container(
              width: 54,
              height: 54,
              decoration: BoxDecoration(color: HalalFoodTheme.primaryGreen.withValues(alpha: 0.10), borderRadius: BorderRadius.circular(14)),
  ...
  Widget _buildOrderCard(Map<String, dynamic> order) {
    ...
    return Card(
      ...
      child: ListTile(
        onTap: () => Navigator.of(context).push(MaterialPageRoute(builder: (_) => OwnerOrderDetailsScreen(order: order))),
        leading: Container(
          width: 44,
          height: 44,
        ...
        title: Text('#$shortId', style: const TextStyle(fontWeight: FontWeight.w800)),
        subtitle: Text(status.replaceAll('_', ' ').toUpperCase(), style: const TextStyle(fontSize: 12, fontWeight: FontWeight.w700)),
            ...
            Expanded(
              child: Column(
                ...
                children: [
                  ...
                  Text(label, maxLines: 2, overflow: TextOverflow.ellipsis, style: const TextStyle(color: HalalFoodTheme.textSecondary, fontSize: 12)),
                ],
              ),
            ),
          ],
