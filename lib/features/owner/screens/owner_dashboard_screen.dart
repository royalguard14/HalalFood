...
class _OwnerDashboardScreenState extends State<OwnerDashboardScreen> {
  ...
  Widget _buildOrderCard(Map<String,dynamic> order){final id=order['id']?.toString()??'',status=order['status']?.toString()??'pending',total=(order['total_amount'] as num?)?.toDouble()??0,shortId=id.length<=8?id:id.substring(0,8);return Card(margin:const EdgeInsets.only(bottom:10),child:ListTile(onTap:()=>Navigator.of(context).push(MaterialPageRoute(builder:(_)=>OwnerOrderDetailsScreen(order:order))),leading:Container(width:44,height:44,decoration:BoxDecoration(color:HalalFoodTheme.primaryGreen.withValues(alpha:.10),borderRadius:BorderRadius.circular(12)),child:const Icon(Icons.receipt_long_rounded,color:HalalFoodTheme.primaryGreen)),title:Text('#$shortId',style:const TextStyle(fontWeight:FontWeight.w800)),subtitle:Text(status.replaceAll('_',' ').toUpperCase(),style:const TextStyle(fontSize:12,fontWeight:FontWeight.w700)),trailing:Text('₱${total.toStringAsFixed(2)}',style:const TextStyle(fontWeight:FontWeight.w800))));}
